...
output["Minutes ago"] = [(json.loads(kv)[0]-latest)/60000.0 for kv in res[0]]

for idx, sensordata in enumerate(res):
        output[sensors[idx]] = [json.loads(kv)[1] for kv in sensordata]

from operator import add
output["Total"] = list(map(     add, list(map(  add, output[sensors[0]], output[sensors[1]])), output[sensors[2]] ))
